...
constructor( { data, visual, dataKey, visualKey } ={} ) {
  ...
  */  
  ...
  // Helper function to get the joined dataRow
	...
	getJoinByKey(key, protoString){	
			...
			return key ?																										
							this.join[protoString == "index" ? "findIndex" : "find"](function(join){ 
																						return join.dataRow[dataKey] == key;
																					}) //function
						:	null;
  ...
  */   
  addToolTip(renderer, camera, getToolTipText, handleDblClick) {
    ...
    const onMouseEvent = (event) => {
      ...
      const handleMouseIntersects = (mouse, intersect) => {
        ...
        if (event.type == "dblclick" && handleDblClick)
          handleDblClick(join, this);
        
  
      };  // handleMouseIntersects     

      if (intersects[0]) {
        
        handleMouseIntersects(mouse, intersects[0]);
   ...
   renderer.domElement.addEventListener( "mousemove", onMouseEvent, false ); 
   renderer.domElement.addEventListener( "mouseleave", onMouseEvent, false);  
    
   if (handleDblClick) 
      renderer.domElement.addEventListener( "dblclick", onMouseEvent, false );
