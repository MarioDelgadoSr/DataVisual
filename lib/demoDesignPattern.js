...
var data = [																												// Data establishing a 'value' for each salvageable cube
...
function fnVisualize(data, seaScape){
	...
	var orignVector = new THREE.Vector3(0,0,0);																				// Sets origin for distance calculations	
	...
	seaScapeDataVisual.join.forEach(function (join){     																	// Iterate through each joinRow and extend round trip distance  
		...
		join.originToVisualObjDistance  = orignVector.distanceTo(join.visualObj.position);        									// https://threejs.org/docs/index.html#api/en/math/Vector3.distanceTo
...
function fnDisplay(seaScapeDataVisual){
	...
	var gui;
	fnBuildDatGui();																										//Build dat.gui with viusalization	
	...
	// Visualization of retrieval path
	var material = new THREE.LineBasicMaterial({
												color: "#" + seaScapeDataVisual.join[0].visualObj.material.color.getHexString()
												});
	...
	//Show the aquaRobot retrieval pattern; highest ranked factor to lowest	
	var index = 0;	
	setInterval(function(){
				
				if (seaScapeDataVisual.join[index].visible){																		//The join's visibiblity, not visualObj
	...
	function onMouseDown( event ) {
		...
		var intersects = raycaster.intersectObjects( seaScapeDataVisual.visual.scene.children ); 									// calculate objects intersecting the picking ray
		...
		intersects.some(function(intersect){
			...
			if (join){																												// Update dat Gui with selection indicator
				...
				d3.select(gui.domElement)
					.selectAll(".property-name")
					.text(function(){ 
						var searchText = d3.select(this).text().split("****");  
						cubeName = searchText[searchText.length-1].trim();  ;  
	...
	function fnBuildDatGui(){
				
		var guiDisplay = {"Show Axis": false, "Show Hidden": false };		

		gui = new dat.GUI();
		
			
		var fnShowHidden = gui.add(guiDisplay, 'Show Hidden');
		...
		var folder = gui.addFolder("Click an object to indentify");														// Utilizes seaScapeDataVisual.getJoinByUUID(intersect.object.uuid)
		
		var folder = gui.addFolder("Value");
		folder.open();
		...
		var folder = gui.addFolder("Salvage Order");
		folder.open();
