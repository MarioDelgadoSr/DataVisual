...
function fnDisplay(seaScapeDataVisual){
	...
	// aquaRobot
	var botGeometry = new THREE.IcosahedronGeometry(1);
	var botMaterial = new THREE.MeshBasicMaterial();
	var aquaBot = new THREE.Mesh(botGeometry, botMaterial);
	...
	seaScapeDataVisual.addToolTip(  renderer, 
                                  camera, 
                                  undefined, 
		...
		seaScapeDataVisual.join.forEach(function(join){
			...
			if (join.visible){
				...
				d3.select(folder.__controllers[folder.__controllers.length -1].domElement)  										// A little hack of dat Gui		
					...
					.style("background-color", join.color);																// Then use data driven visualization
